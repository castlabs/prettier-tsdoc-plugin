...
- **Validation Rules**:
  - API Extractor reports `ae-missing-release-tag` for exported items without
    tags
  - Logical compatibility rules apply (e.g., `@public` function can't return
    `@beta` type)
...
- **Container Analysis**: Detect if comment belongs to class member vs top-level
  construct
...
- **`inheritanceAware`**: Skip tagging when inheritance applies (default:
  `true`)
...
- **Container Inheritance**: Class/interface members are skipped (inherit from
  container)
